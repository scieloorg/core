...
@celery_app.task(bind=True, name="task_delete_article_without_pp_xml")
def task_delete_article_without_pp_xml(self):
    """
    Remove artigos órfãos que não possuem XML correspondente no PidProvider.

    Deleta artigos que não têm pp_xml vinculado e para os quais não existe
    um registro PidProviderXML correspondente no sistema.
    """
    delete_article_without_pp_xml()


def delete_article_without_pp_xml():
    """
    Remove artigos sem PidProviderXML correspondente.

    Verifica artigos sem pp_xml e os remove se não existir um registro
    PidProviderXML com o mesmo pid_v3, indicando que são registros órfãos.
    """
    for article in Article.objects.filter(pp_xml__isnull=True).iterator():
        try:
            PidProviderXML.objects.get(v3=article.pid_v3)
        except PidProviderXML.DoesNotExist:
            article.mark_as_delete()


@celery_app.task(bind=True, name="task_mark_incomplete_article_to_load")
def task_mark_incomplete_article_to_load(self):
    """
    Marca XMLs para reprocessamento quando seus artigos estão inválidos.

    Identifica artigos marcados como inválidos e atualiza o status de seus
    PidProviderXMLs correspondentes para PPXML_STATUS_TODO, forçando reprocessamento.
    """
    mark_incomplete_article_to_load()


def mark_incomplete_article_to_load():
    """
    Atualiza status de PidProviderXMLs para reprocessar artigos inválidos.

    Encontra artigos com valid=False e marca seus PidProviderXMLs correspondentes
    com status TODO para que sejam reprocessados na próxima execução.
    """
    articles = Article.objects.filter(valid=False).values("pid_v3")
    PidProviderXML.objects.filter(
        ~Q(proc_status=PPXML_STATUS_TODO),
        v3__in=Subquery(articles),
    ).update(proc_status=PPXML_STATUS_TODO)
