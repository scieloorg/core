...
class LocationTest(TestCase):
    def test_standardize_returns_location_object(self):
        text_country = "BR, Brasil"
        text_state = "SP"
        text_city = " Santos "
        user, created = User.objects.get_or_create(username="adm")
        result = models.Location.standardize(text_city, text_state, text_country, user)

        for i, item in enumerate(result):
            with self.subTest(i):
                self.assertIsInstance(item["location"], models.Location)
                self.assertEqual("Brasil", item["location"].country.name)
                self.assertEqual("BR", item["location"].country.acronym)
                self.assertEqual(None, item["location"].state.name)
                self.assertEqual("SP", item["location"].state.acronym)
                self.assertEqual("Santos", item["location"].city.name)

    def test_standardize_returns_location_dict(self):
        text_country = "BR, Brasil"
        text_state = "SP"
        text_city = " Santos "
        result = models.Location.standardize(text_city, text_state, text_country)

        for i, item in enumerate(result):
            with self.subTest(i):
                self.assertIsInstance(item["location"], dict)
                self.assertEqual("Brasil", item["location"]["country"]["name"])
                self.assertEqual("BR", item["location"]["country"]["code"])
                self.assertEqual(None, item["location"]["state"].get("name"))
                self.assertEqual("SP", item["location"]["state"]["code"])
                self.assertEqual("Santos", item["location"]["city"]["name"])

    def test_standardize_returns_location_country_city_object(self):
        text_country = "BR, Brasil"
        text_state = None
        text_city = " Santos "
        user, created = User.objects.get_or_create(username="adm")
        ...
        for i, item in enumerate(result):
            with self.subTest(i):
                ...
                self.assertEqual("Brasil", item["location"].country.name)
                self.assertEqual("BR", item["location"].country.acronym)
                self.assertEqual(None, item["location"].state)
                self.assertEqual("Santos", item["location"].city.name)

    def test_standardize_returns_location_country_city_dict(self):
        text_country = "BR, Brasil"
        text_state = None
        text_city = " Santos "
        result = models.Location.standardize(text_city, text_state, text_country)
        ...
        for i, item in enumerate(result):
            with self.subTest(i):
                ...
                self.assertEqual("Brasil", item["location"]["country"]["name"])
                self.assertEqual("BR", item["location"]["country"]["code"])
                self.assertEqual(None, item["location"].get("state"))
                self.assertEqual("Santos", item["location"]["city"]["name"])
