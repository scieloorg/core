...
from issue.articlemeta.loader import (
    create_issue_from_am_issue,
    get_issue_data_from_am_issue,
    harvest_and_load_issue,
    harvest_issue_identifiers,
    load_bibliographic_strips,
    load_issue_sections,
    load_issue_titles,
)
        ...
        for acron3 in collection_acronyms:
            try:
                ...
                for issue_identifier in harvest_issue_identifiers(
                    ...
                    try:
                        logger.info(
                            f"Scheduling load for issue {issue_identifier.get('code')} in collection {acron3}"
                        )
    ...
    try:
        ...
        if collection_acron:
            try:
                ...
                raise ValueError(
                    f"Collection with acron3 '{collection_acron}' not found"
                )

        if issue_status:
            if issue_status not in ["pending", "todo", "done"]:
                raise ValueError(
                    "issue_status must be one of: 'pending', 'todo', 'done'"
                )
        ...
        for am_issue in am_issues.iterator():
            try:
                ...
                if existing_issue and force_update:
                    ...
                    logger.info(
                        f"Updating existing Issue {existing_issue.id} from AMIssue {am_issue.id}"
                    )
                    ...
                    load_bibliographic_strips(
                        user, existing_issue, am_issue, issue_data
                    )
        ...
        logger.info(
            f"Task completed. Processed: {stats['processed']}, "
