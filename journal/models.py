...
class Journal(CommonControlField, ClusterableModel):
    ...
    def _migrate_history_to_raw(self, history_item):
        """
        Migra dados de *History.institution para os campos raw_* de RawOrganizationMixin.
        
        Este método extrai dados de um objeto institution (Publisher, Owner, Sponsor, ou CopyrightHolder)
        e popula os campos raw_* correspondentes no objeto history_item, então define institution como None.
        
        Args:
            history_item: Um objeto *History (PublisherHistory, OwnerHistory, SponsorHistory, ou CopyrightHolderHistory)
        
        Returns:
            history_item: O objeto *History atualizado com dados migrados
        """
        if not history_item.institution:
            # Não há instituição para migrar
            return history_item
        ...
        if institution_obj.institution_identification:
            ...
            parts = []
            if institution_obj.institution_identification.name:
                parts.append(institution_obj.institution_identification.name)
            if institution_obj.institution_identification.acronym:
                parts.append(f"({institution_obj.institution_identification.acronym})")
            if institution_obj.level_1:
                parts.append(institution_obj.level_1)
            if institution_obj.level_2:
                parts.append(institution_obj.level_2)
            if institution_obj.level_3:
                parts.append(institution_obj.level_3)
        ...
        if institution_obj.location:
            location = institution_obj.location
            ...
            if hasattr(location, 'country') and location.country:
                ...
                if hasattr(location.country, 'acron3'):
                    ...
                    history_item.raw_country_code = location.country.acronym
    ...
    def migrate_publisher_history_to_raw(self):
        """
        Migra dados de publisher_history.institution para os campos raw_* de RawOrganizationMixin.
        
        Itera sobre todos os PublisherHistory relacionados a este journal e migra dados
        de institution para os campos raw_*, definindo institution como None após a migração.
        
        Returns:
            list: Lista de objetos PublisherHistory migrados
        """
        migrated_items = []
        for history_item in self.publisher_history.all():
            self._migrate_history_to_raw(history_item)
            migrated_items.append(history_item)
        return migrated_items

    def migrate_owner_history_to_raw(self):
        """
        Migra dados de owner_history.institution para os campos raw_* de RawOrganizationMixin.
        
        Itera sobre todos os OwnerHistory relacionados a este journal e migra dados
        de institution para os campos raw_*, definindo institution como None após a migração.
        
        Returns:
            list: Lista de objetos OwnerHistory migrados
        """
        migrated_items = []
        for history_item in self.owner_history.all():
            self._migrate_history_to_raw(history_item)
            migrated_items.append(history_item)
        return migrated_items

    def migrate_sponsor_history_to_raw(self):
        """
        Migra dados de sponsor_history.institution para os campos raw_* de RawOrganizationMixin.
        
        Itera sobre todos os SponsorHistory relacionados a este journal e migra dados
        de institution para os campos raw_*, definindo institution como None após a migração.
        
        Returns:
            list: Lista de objetos SponsorHistory migrados
        """
        migrated_items = []
        for history_item in self.sponsor_history.all():
            self._migrate_history_to_raw(history_item)
            migrated_items.append(history_item)
        return migrated_items

    def migrate_copyright_holder_history_to_raw(self):
        """
        Migra dados de copyright_holder_history.institution para os campos raw_* de RawOrganizationMixin.
        
        Itera sobre todos os CopyrightHolderHistory relacionados a este journal e migra dados
        de institution para os campos raw_*, definindo institution como None após a migração.
        
        Returns:
            list: Lista de objetos CopyrightHolderHistory migrados
        """
        migrated_items = []
        for history_item in self.copyright_holder_history.all():
            self._migrate_history_to_raw(history_item)
            migrated_items.append(history_item)
        return migrated_items
