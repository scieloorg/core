...
class GenericJournalViewSet(viewsets.ModelViewSet):
    ...
    queryset = models.Journal.objects.prefetch_related(
        Prefetch(
            "crossmark_policy",
            queryset=models.CrossmarkPolicy.objects.select_related(
                "language", "journal__official"
            ).prefetch_related(
                Prefetch(
...
class JournalViewSet(GenericJournalViewSet):
    def get_queryset(self):
        ...
        if query_params.get("formats") == "articlemeta":
            return (
                query.filter(scielojournal__journal__isnull=False)
                .filter(**params)
                .annotate(collection_acron=F("scielojournal__collection__acron3"))
                .order_by("created")
                .distinct()
            )
...
class CrossmarkPolicyViewSet(viewsets.ModelViewSet):
    ...
    **GET /api/v1/crossmarkpolicy/{id}/** — retrieve a specific crossmark policy
    ...
    def get_queryset(self):
        ...
        issn_filter = None
        if issn := query_params.get("issn"):
            issn_filter = Q(journal__official__issn_electronic=issn) | Q(
                journal__official__issn_print=issn
            )
