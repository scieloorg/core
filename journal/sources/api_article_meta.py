...
def get_articlemeta_format(obj):
    ...
    add_to_result("collection", scielo_journal.collection.acron3 if scielo_journal and scielo_journal.collection else None)
    ...
    add_to_result("v50", scielo_journal.status.lower() if scielo_journal.status else None)
    add_items("v62", [ch.get_institution_name for ch in obj.copyright_holder_history.all()])
    add_to_result("v66", obj.ftp)
    add_to_result("v67", obj.user_subscription)
    add_to_result("v68", scielo_journal.journal_acron if scielo_journal and scielo_journal.journal_acron else None)
    add_to_result("v69", obj.journal_url)
    add_to_result("v85", obj.vocabulary.acronym)
    add_to_result("v110", obj.subtitle)
    add_to_result("v117", obj.standard.code if obj.standard and obj.standard.code else None)
    add_to_result("v130", obj.section)
    add_items("v140", [sponsor.get_institution_name for sponsor in obj.sponsor_history.all()])
    ...
    if obj.official:
        ...
        add_to_result("v151", obj.official.iso_short_title if obj.official.iso_short_title else None)
        add_items("v230", [pt.text for pt in obj.official.parallel_titles if pt.text])
        add_to_result("v301", obj.official.initial_year if obj.official.initial_year else None)
        add_to_result("v302", obj.official.initial_volume if obj.official.initial_volume else None)
        add_to_result("v303", obj.official.initial_number if obj.official.initial_number else None)        
        ...
        add_to_result("v305", obj.official.final_volume if obj.official.final_volume else None)
        add_to_result("v306", obj.official.final_number if obj.official and obj.official.final_number else None)
        ...
        if obj.official.old_title.all():
            add_items("v610", [old_title.title for old_title in obj.official.old_title.all()])
    ...
    result["v901"] = [{"l": mission.language.code2, "_": mission.get_text_pure} 
