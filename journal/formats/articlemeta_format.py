...
class ArticlemetaJournalFormatter:
    ...
    def __init__(self, obj, collection):
        ...
        self._titles_in_database_medline_secs = None
    ...
    def titles_in_database_medline_secs(self):
        """Cache manual (mesmo padrão de scielo_journal) para evitar problemas do lru_cache em instância."""
        if self._titles_in_database_medline_secs is not None:
            return self._titles_in_database_medline_secs

        self._titles_in_database_medline_secs = list(
    ...
    def _format_basic_info(self):
        ...
        add_to_result(
            "v117",
            self.obj.standard.code if self.obj.standard and self.obj.standard.code else None,
            self.result,
        )
    ...
    def _format_title_journal_info(self):
        """Informações do Title Journal"""
        add_to_result("v150", self.obj.short_title, self.result)
        if iso_short_title := getattr(self.official, 'iso_short_title', None):
    ...
    def _format_scielo_journal_info(self):
        ...
        if self.scielo_journal:
            ...
            key_to_value = {
                "v50": self.scielo_journal.status or None,
                "v68": journal_acron,
                "v400": issn_scielo,
                "v880": issn_scielo,
                "v930": journal_acron.upper() if journal_acron else None,
            }
    ...
    def _format_publisher_info(self):
        """Informações do publisher/owner"""
        try:
            owner_data = self.obj.owner_data
        except AttributeError:
            owner_data = {}
            first_owner = (
                self.obj.owner_history
                .select_related(
            ...
            if first_owner:
                owner_data["country_acronym"] = first_owner.institution_country_acronym
                owner_data["state_acronym"] = first_owner.institution_state_acronym
                owner_data["city_name"] = first_owner.institution_city_name
    ...
    def _format_indexing_info(self):
        ...
        medline_data = []
        secs_data = []
        for t in titles_in_db:
            acronym_lower = t.indexed_at.acronym.lower()
            if acronym_lower == "medline":
                medline_data.append(t)
            elif acronym_lower == "secs":
                secs_data.append(t)
    ...
    def _format_issn_type(self, issn_print):
        """
        ATENÇÃO: revisar a lógica de negócio.
        No código original, se issn_print == issn_scielo, atribuía 'ONLIN',
        o que parece invertido. Mantido o comportamento original aqui,
        mas marcado para revisão.
        """
        if self.scielo_journal:
            if issn_print == self.scielo_journal.issn_scielo:
                # TODO: verificar se deveria ser "PRINT" em vez de "ONLIN"
                add_to_result("v35", "ONLIN", self.result)
            else:
                add_to_result("v35", "PRINT", self.result)
    ...
    def _format_journal_history_entry(self, subfield_a, subfield_b):
        """Formata um registro de histórico do journal."""
        entry = {
            "_": "",
            "a": subfield_a,
            "b": "C",
    ...
    def _format_journal_history(self):
        ...
        for jh in journal_history:
            ...
            # subfield_b determinado exclusivamente pelo evento atual
            subfield_b = ""
            if jh.interruption_reason:
                subfield_b = "D" if jh.interruption_reason == "ceased" else "S"

            if jh.event_type in ("ADMITTED", "INTERRUPTED"):
                entry = self._format_journal_history_entry(
