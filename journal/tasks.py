...
def load_journal_logo(self, user_id=None, username=None):
    for scielo_journal in SciELOJournal.objects.all():
        ...
        fetch_and_process_journal_logo.apply_async(
            kwargs=dict(
...
def fetch_and_process_journal_logo(
    ...
    try:
        ...
        JournalLogo.create_or_update(journal=journal, logo=img_wagtail, user=user)
    except Exception as e:
        exc_type, exc_value, exc_traceback = sys.exc_info()
        UnexpectedEvent.create(
            exception=e,
...
def assign_logo_to_all_journals(self, user_id=None, username=None):
    for journal in Journal.objects.all():
        update_journal_logo_if_exists.apply_async(
            kwargs=dict(
