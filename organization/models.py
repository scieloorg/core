...
class OrganizationalLevel(BaseOrganizationalLevel, CommonControlField):
    ...
    @classmethod
    def create_or_update(
        cls, user, organization, level_1=None, level_2=None, level_3=None
    ):
        """
        ...
        if not organization:
            raise ValueError(
                "OrganizationalLevel.create_or_update requires organization"
            )
...
class RawOrganization(
    BaseOrganization, BaseOrganizationalLevel, CommonControlField, ClusterableModel
):
    """
    ...
    def autocomplete_custom_queryset_filter(search_term):
        return RawOrganization.objects.filter(
            Q(country__icontains=search_term) | Q(original__icontains=search_term)
        )
    ...
    class Meta:
        ...
        unique_together = [
            ("original", "country", "state", "city", "level_1", "level_2", "level_3")
        ]
        indexes = [
    ...
    def add_organizational_level(
        self, user, organizational_level, match_status="manual"
    ):
        """
